...
<%
	...
	//DB연결
		...
		try{
			...
			pstmt=conn.prepareStatement("insert into member2 (id,password, name) values(?,?,?)");
			// 첫번째 물음표에는 사용자가 입력한id값(request.getParameter("id"))을 설정
			pstmt.setString(1, id);
			// 두번째 물음표에는 사용자가 입력한password값(request.getParameter("pw"))을 설정
			pstmt.setString(2, pw);
			// 세번째 물음표에는 사용자가 입력한addr값(request.getParameter("addr"))을 설정
			...
			if(result!=0){
				...
				// 회원가입 화면으로 이동
				out.println("<script>");
				out.println("location.href='member2.jsp'");
				out.println("</script>");
			}
